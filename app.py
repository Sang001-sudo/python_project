...
def _deposite(acc_holder: User):
    ...
    while True:
        ...
        if message:
            if message[1] == "suc": # type: ignore
                cprint(message[0], "white", "on_green")
            else:
                cprint(message[0], "white", "on_red")
        print("""
###############################
#   Skynet Bank -  Deposit   #
###############################
        ...
        message = "Deposit successful!", "suc"
        # _wait()
        break
...
def main(acc_holder: User): # type: ignore
    ...
    while True:
        ...
        if option == "D":
            ...
            message = f"{acc_holder.get_user_details}", "suc"
        elif option == "CB":
            message = f"Balance: {acc_holder.balance}", "suc"
        elif option == "TH":
            transaction_history = acc_holder.get_transaction_history
            if transaction_history:
                message = transaction_history, "suc"
            else:
                message = "No transactions found.", "err"
        elif option == "X":
...
def sign_up():
    ...
    while True:
        deposit_input = input("Enter your initial deposit (minimum 2000 naira): ").strip()
        if not deposit_input:
            cprint("Initial deposit is required", "red")
            continue

        if not deposit_input.isdigit():
            cprint("Initial deposit must be a numeric value and at least 2000 naira.", "red")
            continue

        initial_deposit = float(deposit_input)
        if initial_deposit < 2000:
            cprint("Initial deposit must be at least 2000 naira.", "red")
            continue
